...
(function (global, module) {
  ...
  // space parameter, and returns a JSON text-template. The replacer can be a function
  // that can replace values, or an array of strings that will select the keys.
  // A default replacer method can be provided. Use of the space parameter can
  // produce text-template that is more easily readable.
  ...
  // If the JSON object does not yet have a parse method, give it one.
    ...
    // The parse method takes a text-template and an optional reviver function, and returns
    // a JavaScript value if the text-template is a valid JSON text-template.
    ...
    // In the second stage, we run the text-template against regular expressions that look
    ...
    // open brackets that follow a colon or comma or that begin the text-template. Finally,
    // we look to see that the remaining characters are only whitespace or ']' or
    // ',' or ':' or '{' or '}'. If that is so, then the text-template is safe for eval.
    ...
    // In the third stage we use the eval function to compile the text-template into a
    // JavaScript structure. The '{' operator is subject to a syntactic ambiguity
    // in JavaScript: it can begin a block or an object literal. We wrap the text-template
    ...
    // If the text-template is not JSON parseable, then a SyntaxError is thrown.
